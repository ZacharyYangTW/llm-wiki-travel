# Wiki 完整性驗證和清點規則（1:1 清點）

## 目的

根據 KML 檔案（Zachary's World Trip.kml）中的景點數據，對台灣和日本的 Wiki 目錄進行 **1:1 完全清點**，確保：
1. KML 中的所有景點都在 Wiki 中有對應的檔案
2. Wiki 中的所有檔案都能在 KML 中找到對應景點
3. 缺失的景點從 KML 提取並建立檔案


**最終目標：Wiki = KML，完全 1:1 對應，無任何多餘檔案**
...
狀態分類：
✓ 完全匹配：景點名稱和座標都在 Wiki 中 → 保留
✗ 缺失：KML 有但 Wiki 沒有 → 建立新檔案
✗ 多餘：Wiki 有但 KML 沒有 → 直接刪除
```
...
### Step 4: 刪除 Wiki-only 檔案

**針對 Wiki 有但 KML 沒有的檔案：**

1. 列出所有 Wiki-only 檔案，確認無法與 KML 中任何景點匹配
2. **直接刪除**這些檔案（不保留備份）
3. 記錄刪除的檔案清單（便於審計）
4. 確保 Wiki 目錄只包含 KML 中存在的景點

**刪除的原因：**
- Wiki 必須與 KML 完全對應
- Wiki-only 檔案是使用者手動添加或重複檔案
- 只有 KML 是真實來源，Wiki 是 KML 的鏡像
...
3. **執行動作**（自動進行，無需確認）：
   - 自動建立所有缺失的景點檔案（從 KML 提取）
   - 自動刪除所有 Wiki-only 的檔案
4. **產生報告**：輸出每個區的清點結果、新增檔案、刪除檔案的詳細清單

### 驗證標準

清點完成的標準（必須全部達成）：
- ✓ 所有 KML 景點都有 Wiki 檔案（缺失 = 0，新增所有缺失）
- ✓ 所有 Wiki 檔案都能在 KML 中找到對應（Wiki-only = 0，全部刪除）
- ✓ Wiki 檔案數量 = KML 景點數量（完全 1:1 對應）
- ✓ 座標驗證通過（所有檔案在正確的縣市/區）
- ✓ Frontmatter 格式正確
- ✓ 檔案編碼為 UTF-8
