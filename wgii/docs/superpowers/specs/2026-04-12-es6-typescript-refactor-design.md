...
WGII（World Geographic Information Integration）是一个世界地理資訊集成工具，用于生成和整理各国GeoJSON資料。当前版本0.0.1使用CommonJS和老旧依赖，需要现代化重构。
...
| 代碼现代化 | ES Modules、async/await、原生Promise、原生fetch |
| 架构简化 | 移除MySQL依赖，使用纯JSON文件存储 |
| 灵活使用 | 支持CLI工具、npm包引用、資料源三种使用方式 |
...
| `src/model.js` | `src/core/country.ts` | 國家資料模型 |
| `src/sync.js` | `src/core/geojson.ts` | 資料同步逻辑 |
...
國家資訊JSON格式保持与现有 `countries.info.json` 兼容。
...
| 模块系统 | ES Modules | Node 20原生支持，符合现代標準 |
