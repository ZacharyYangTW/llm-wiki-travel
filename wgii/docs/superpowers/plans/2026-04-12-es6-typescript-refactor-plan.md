...
| `src/types/country.d.ts` | 國家資料类型定义 |
...
| `src/core/country.ts` | 國家資訊管理 |
| `src/core/geojson.ts` | GeoJSON資料处理 |
| `src/storage/json-store.ts` | JSON文件读写 |
| `src/storage/resource.ts` | 源資料管理 |
...
```json
{
  "name": "wgii",
  "version": "1.0.0",
  "description": "World Geographic Information Integration - 世界地理資訊集成",
  "type": "module",
  "main": "dist/index.js",
...
export interface CountryInfo {
  ...
  /** ISO3166-1 三位字母码 */
  countryCode: string;
  /** 國家类型: Sovereign country, Member State 等 */
  countryType: string;
  /** 中文名称 */
  nameChinese: string;
  /** 中文简称 */
...
cd m:/programer/sourcecode/wgii
git add src/types/
git commit -m "feat: 添加GeoJSON和國家資料类型定义"
```
...
export class ResourceManager {
  /**
   * 加载國家GeoJSON源資料
   * @param countryCode 國家代碼(ISO3166-1三位字母码)
   */
...
- [ ] **Step 2: Commit源資料管理模块**

```bash
cd m:/programer/sourcecode/wgii
git add src/storage/resource.ts
git commit -m "feat: 添加源資料管理模块"
```
...
- [ ] **Step 2: Commit國家資訊管理模块**

```bash
cd m:/programer/sourcecode/wgii
git add src/core/country.ts
git commit -m "feat: 添加國家資訊管理模块"
```
...
World Geographic Information Integration - 世界地理資訊集成
...
`dist/` 目录包含生成的GeoJSON資料：

```
dist/
├── countries.info.json        # 國家基础資訊
├── CHN/
...
|------|------|
| `loadAll()` | 加载所有國家資訊 |
| `findByCode(code)` | 根据ISO3166-1代碼查询 |
...
| §6 CLI设计 | Task 14 | ✓ |
| §7 npm包API | Task 15 | ✓ |
| §9 資料兼容 | Task 11 (保持dist结构) | ✓ |
| §10 package.json/tsconfig | Task 1 | ✓ |

### 2. Placeholder扫描

无TBD/TODO占位符，所有步骤包含完整代碼。
